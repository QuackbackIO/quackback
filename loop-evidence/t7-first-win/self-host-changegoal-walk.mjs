#!/usr/bin/env bun
/**
 * Local self-host support + Help Center first-win via Change goal UI.
...
const OUT = '/home/james/quackback-wt/saas-merge/loop-evidence/t7-first-win/self-host-changegoal'
mkdirSync(OUT, { recursive: true })
const ORIGIN = 'http://localhost:3000'

const facts = { at: new Date().toISOString(), origin: ORIGIN, method: 'change-goal-ui', steps: [], errors: [] }
...
rec('signin', { status: signin.status, user: signinJson?.user?.email === 'demo@example.com' })
if (signin.status !== 200) throw new Error(`signin ${signin.status}`)
...
function summarize(body) {
  return {
    ...
    goal: (body.match(/current goal\s+([A-Za-z ]+)/i) || [])[1]?.trim() || null,
    firstWinTitle:
      (body.match(
    ...
    readyPrimary:
      (body.match(/connect messenger|open help center|open feedback|invite a teammate/i) ||
        [])[0] || null,
...
async function openLaunch() {
  await page.goto(`${ORIGIN}/admin/getting-started`, { waitUntil: 'networkidle', timeout: 30000 })
...
async function setGoal(label) {
  ...
  if (await change.isDisabled()) throw new Error('Change goal disabled')
  await change.click()
  const save = page.getByRole('button', { name: /use this goal/i })
  await save.waitFor({ state: 'visible', timeout: 10000 })
  await page.getByRole('radiogroup', { name: /workspace goal/i }).getByText(label, { exact: true }).click()
  await save.click()
  await page.getByRole('heading', { name: new RegExp(`^${label}$`, 'i') }).waitFor({ timeout: 15000 })
...
facts.supportOk = Boolean(support?.firstWinTitle && /conversation/i.test(support.firstWinTitle) && support.upAndRunning)
facts.helpCenterOk = Boolean(hc?.firstWinTitle && /article/i.test(hc.firstWinTitle) && hc.upAndRunning)
...
console.log(
  JSON.stringify({
    ...
    errors: facts.errors,
    support: support && { goal: support.goal, firstWinTitle: support.firstWinTitle, readyPrimary: support.readyPrimary },
    hc: hc && { goal: hc.goal, firstWinTitle: hc.firstWinTitle, readyPrimary: hc.readyPrimary },
  })
)
