...
/**
 * Convert a raw Better-auth member ID to MemberId format
 *
 * Better-auth stores member IDs as plain text (nanoid format).
 * This helper adds the 'member_' prefix for use in our typed system.
 *
 * @param rawId - The raw member ID from Better-auth (e.g., "abc123xyz")
 * @returns A typed MemberId (e.g., "member_abc123xyz")
 *
 * @example
 ...
 * // => 'member_abc123xyz'
 */
export function toMemberId(rawId: string): TypeId<'member'> {
  return `member_${rawId}` as TypeId<'member'>
}

/**
 * Convert a raw Better-auth user ID to UserId format
 *
 * @param rawId - The raw user ID from Better-auth
 * @returns A typed UserId
 */
export function toUserId(rawId: string): TypeId<'user'> {
  return `user_${rawId}` as TypeId<'user'>
}

/**
 * Convert a raw Better-auth organization ID to OrgId format
 *
 * @param rawId - The raw organization ID from Better-auth
 * @returns A typed OrgId
 */
export function toOrgId(rawId: string): TypeId<'org'> {
  return `org_${rawId}` as TypeId<'org'>
}
