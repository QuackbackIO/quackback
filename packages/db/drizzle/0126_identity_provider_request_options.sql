...
--
-- The scopes column already exists. These two are its siblings: `prompt` and
-- the token-endpoint authentication method were both fixed in code. Null means
-- the default in each case, matching how `scopes` already behaves, so no
