...
/**
 * 0260 demotes every sending domain that was verified before ownership existed.
 *
...
const MIGRATION_SQL = readFileSync(
  join(__dirname, '../../drizzle/0260_sending_domain_reverify.sql'),
  'utf8'
)
  ...
  await db
    .transaction(async (tx) => {
      await tx.execute(sql`
        CREATE TABLE "_m0260_email_sending_domains" (
...
async function rows(tx: Parameters<Parameters<Database['transaction']>[0]>[0]): Promise<Row[]> {
  const result = await tx.execute<Row>(sql`
    ...
    FROM "_m0260_email_sending_domains" ORDER BY "domain"
  `)
  return result as unknown as Row[]
}

describe.skipIf(!dbAvailable)('migration 0260 sending-domain re-verification', () => {
  it('demotes a row verified by the check that could not tell an owner from anyone', async () => {
    await withScratch(async (tx) => {
      await tx.execute(sql`
        INSERT INTO "_m0260_email_sending_domains"
  ...
  it('leaves pending and failed rows alone', async () => {
    await withScratch(async (tx) => {
      await tx.execute(sql`
        INSERT INTO "_m0260_email_sending_domains" ("domain", "status", "dns_records")
  ...
  it('handles a row with no records at all', async () => {
    ...
    await withScratch(async (tx) => {
      await tx.execute(sql`
        INSERT INTO "_m0260_email_sending_domains" ("domain", "status", "verified_at")
