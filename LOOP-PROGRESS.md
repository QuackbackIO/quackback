...
- Workspace tip: `e56d00b9a` (webhooks/workflows/macros/MCP upgrade
  offer). Live web still `48468c28` / `sha256:b358f971…` (`5834951f7`).
  Docker `31898534925` queued for full sha
  `e56d00b9ac8114816cfabc014e1e4479b6d25b0f`. Verify / §H last signed
  on the older `895b942d` pair.
...
- App `saas` tip `e56d00b9a` is **not live**. Fleet is the current unit:
  Docker `31898534925` (queued). Do not start P3/P4 until this digest
  is live and a critic has signed it.
...
| Webhooks/workflows/macros/MCP offer | app          | `e56d00b9a`                                                                       | **no** Docker `31898534925` queued         | Free workspace: create webhook / new workflow / enable MCP opens UpgradeModal; macros page is in-route UpgradeScreen. Mixed Developers page stays up. Cloud-off shows no upgrade chrome. Server 402s remain. Tests 27/27.                      |
...
**Fleet note:** one deploy thread. Live pair is still app `48468c28` /
`sha256:b358f971…` (`5834951f7`) and CP `4a5ea8d7` (`4ad81fc`).
Customer-visible tip `e56d00b9a` is undeployed. Docker
`31898534925` queued (full sha). After SUCCESS, JSON-patch
`source.image` (web `0b821c4a-00fd-4302-9957-7707645608c3`, project
`bd11fc75-db00-4940-b70c-4bddeed30a9f`, env
`aa05f0e8-eeec-4d72-a0d3-c074ee434568`) and
`railway redeploy --from-source`. Then live critic. Verify / §H
