...
| Plan-matrix critic                 | live + spec  | `loop-evidence/plan-matrix-895b942d.md` + `plan-matrix-free-t7.md` | **PASS** on `895b942d` (historical Growth) | t1a still Pro. t1e is now Scale. Re-sign §H after SSO/IdP if definitions change.                                                                                                                                                               |
...
25. **SSO downgrade.** Scale host is now t1e (`t1e-scale-critic.md`).
    `/sso/new` unlocked. Fail-open unit 3/3. Live IdP add → enforce
    → downgrade still open (no IdP added). Do not start custom
    domains.
