...
| Process                  | Variables                                                                                                                                                                             |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CP web/worker            | `REDIS_URL`, `INTEGRATION_GATEWAY_FORWARD_SECRET` (same random 32+ character secret as fleet), `PLATFORM_SETTINGS_ENCRYPTION_KEY`, `QUACKBACK_CP_SETTINGS_TOKEN`                      |
| Fleet web **and** worker | `QUACKBACK_CONTROL_PLANE_URL=https://app.quackback.io`, `QUACKBACK_CP_SETTINGS_TOKEN`, `INTEGRATION_OAUTH_GATEWAY_URL=https://app.quackback.io`, `INTEGRATION_GATEWAY_FORWARD_SECRET` |
| Self-hosted tenant       | Gateway variables unset; platform credentials entered in settings or environment                                                                                                      |

CP `CP_ROLE=all` runs the HTTP gateway and forwarder. If roles are split, run a `CP_ROLE=worker` process in addition to `web`; the web process alone only enqueues. Redis must be durable and reachable before enabling the provider hooks. OAuth bounce rate limiting also depends on Redis and fails closed on an outage. Shared client/signing credentials are managed in CP at Admin → Settings as one encrypted JSON settings object. Pooled containers fetch the settings once before initialization using a dedicated fleet token. JSON environment keys override container variables; missing keys use the container value and null unsets it. Changes require a container restart.

Pooled tenants use their startup environment snapshot for the 16 shared OAuth apps, with no tenant-DB fallback. If CP cannot be loaded, a new Cloud container refuses startup; existing processes retain their snapshot. Self-hosted instances retain DB/env credentials. Workspace-specific API-token integrations and auth-provider credentials retain their existing source.

## Ordered gates

1. Apply CP migrations `0092_integration_installs.sql` , `0093_integration_credentials.sql` and `0094_platform_settings.sql`, configure the CP encryption key and shared settings-read token and populate the environment-key settings JSON object before tenant rollout, then deploy the internal install API and OAuth bounce. Test real active and unknown hostnames, platform redirect aliases, unchanged callback queries and no-store responses. Compare the final deployed source SHA and image digest to this change.
