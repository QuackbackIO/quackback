...
test.describe('Quinn implemented product acceptance', () => {
  ...
  test('a concurrent guidance deletion preserves the other session draft', async ({
    page,
    context,
  }) => {
    const dialog = await addGuidance(page)
  ...
  test('Guidance filters match the mockup and Knowledge opens Help Center', async ({ page }) => {
    await open(page, '/admin/automation/guidance', 'Guidance')
    const editor = await addGuidance(page)
    await editor.getByRole('button', { name: 'Cancel', exact: true }).click()
    await page.getByPlaceholder('Search guidance').fill('')
    await page.getByRole('button', { name: 'Always', exact: true }).click()
    await expect(page.getByText('Everyday instructions', { exact: true })).toBeVisible()
    await expect(page.getByText(tag + ' legacy', { exact: true })).toBeHidden()
    await page.getByRole('button', { name: 'Situations', exact: true }).click()
    await expect(page.getByText(tag + ' legacy', { exact: true })).toBeVisible()
    await expect(page.getByText('Everyday instructions', { exact: true })).toBeHidden()
    await open(page, '/admin/automation/knowledge', 'Knowledge')
    await page.getByRole('link', { name: 'Manage articles' }).click()
    await expect(page).toHaveURL(/\/admin\/help-center/)
  })
