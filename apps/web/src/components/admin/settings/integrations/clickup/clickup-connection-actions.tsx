...
export function ClickUpConnectionActions({
  ...
  useEffect(() => {
    const searchParams = search as { clickup?: string }
    if (searchParams.clickup !== 'connected') return

    setShowSuccess(true)
    const url = new URL(window.location.href)
    url.searchParams.delete('clickup')
  ...
  const handleConnect = async () => {
    setConnecting(true)
    try {
      const url = await getClickUpConnectUrl()
        ...
        {isConnected && (
          <>
            ...
            <ConfirmDialog
              ...
              description="This will remove the ClickUp integration and stop all task syncing. You can reconnect at any time."
