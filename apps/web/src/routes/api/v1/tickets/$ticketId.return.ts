...
export const Route = createFileRoute('/api/v1/tickets/$ticketId/return')({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        try {
          ...
          const allowed =
            hasPermissionForResource(set, PERMISSIONS.TICKET_ASSIGN_ANY, scope) ||
            hasPermissionForResource(set, PERMISSIONS.TICKET_ASSIGN_SELF, scope)
          if (!allowed) return forbiddenResponse('cannot return this ticket')
          return successResponse(await returnTicket(id, auth.principalId as PrincipalId))
        } catch (error) {
