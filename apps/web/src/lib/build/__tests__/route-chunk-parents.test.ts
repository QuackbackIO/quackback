 ...
 *
 * The tree is generated by a build or a dev server, so a checkout that has run
 * neither has none and these tests skip. The perf job still guards the parse:
 * without the links, the JS request counts go over their ceilings.
 */
...
const haveTree = existsSync(TREE)

describe.skipIf(!haveTree)('readRouteParents', () => {
  let parents: ReturnType<typeof readRouteParents>
  beforeAll(() => {
    parents = readRouteParents(TREE)
  })
...
describe.skipIf(!haveTree)('routeChunksImportTheirParent', () => {
