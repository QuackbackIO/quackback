/**
 * Query-options factory for a live Quinn pending-action row. The inbox
 * approval card (PendingActionCard) polls this for the current status instead
 * of trusting the stale note snapshot (metadata.assistantPendingAction) that
 * announced the proposal — mirrors how conversation-inbox's `thread` factory
 * is the single source of truth for its query key + fetcher.
 */
...
export const assistantPendingActionKeys = {
  ...
  detail: (id: AssistantPendingActionId) =>
    ['admin', 'inbox', 'pending-action', id] as const,
}
