...
interface ApiHandlerContext {
  validation: Extract<ApiWorkspaceResult, { success: true }>
}

type ApiHandler = (request: Request, context: ApiHandlerContext) => Promise<Response>

/**
 * Wrap an API route handler with standardized error handling,
 * workspace validation, and optional role checking.
 *
 * @example
