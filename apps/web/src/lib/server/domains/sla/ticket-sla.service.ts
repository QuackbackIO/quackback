 ...
 */
import { db, and, eq, isNull, sql, tickets, slaEvents } from '@/lib/server/db'
import type { SlaPolicyId, TicketId } from '@quackback/ids'
import { NotFoundError, ValidationError } from '@/lib/shared/errors'
import { getSlaPolicy } from './sla-policy.service'
import { resolveScheduleFor, dueAtForSettle, shiftIso } from './sla.service'
import { addOfficeHoursSeconds } from '../office-hours/office-hours.service'
...
/**
 * The CAS guard shared by every writer of `tickets.sla_applied`
 * (pause/resume/settle/sweep claims): the update only lands while the stamp is
 * still the exact one the caller read from — `appliedAt` identifies which SLA
 ...
 * over this one rather than get overwritten. One helper so every call site
 * shares the predicate shape, mirroring sla.service.ts's slaStampGuard.
 */
 ...
 *  Exported for the sweep module (ticket-sla.sweep.ts), whose breach pass logs
 *  through the same single meta shape. */
...
/**
 * Persist a mutated stamp with NO clock event, guarded by ticketSlaStampGuard
 * on the `(appliedAt, pausedAt)` the caller computed `next` from — the same
 * CAS sla.service.ts's commitStamp documents. Returns whether the write
 * landed; a caller whose guard misses must reload the stamp and recompute
 * before retrying (see recordTicketResolution), rather than clobber a newer
 * concurrent write.
 */
async function commitTicketStamp(
  ticketId: TicketId,
  next: TicketSlaApplied,
  at: Date,
  ...
  const [row] = await db
    ...
    .where(ticketSlaStampGuard(ticketId, guard.appliedAt, guard.pausedAt))
 ...
 *  of sync). Same guarded write as commitTicketStamp — the event is logged
 *  only when the stamp write landed. */
async function commitTicketClockEvent(
  ...
  if (!(await commitTicketStamp(ticketId, next, at, guard))) return false
  await insertTicketClockEvent(ticketId, next.policyId, kind, dueAt, at)
  return true
}
 ...
 * fan-out of its linked tickets', never a resolution clock). Returns the
 * stamped SLA, or null for the no-TTR no-op.
 */
export async function applySlaToTicket(
  ...
  const [ticket] = await db
    .select({ id: tickets.id, type: tickets.type })
    .from(tickets)
 ...
 * call read) so a concurrent apply/pause/resume/settle doesn't get clobbered;
 * a guard miss quietly skips rather than overwriting a newer stamp (the same
 * trade-off sla.service.ts's pauseSlaOnSnooze documents). A no-op without an
...
export async function pauseTicketSlaOnPending(
  ...
  const next: TicketSlaApplied = { ...applied, pausedAt: at.toISOString() }
  const [row] = await db
    .update(tickets)
    .set({ slaApplied: next, updatedAt: at })
    .where(ticketSlaStampGuard(ticketId, applied.appliedAt, null))
    .returning({ id: tickets.id })
  if (!row) return
 ...
 * resume makes — see sla.service.ts's resumeSlaFromSnooze). Same
 * guarded-UPDATE approach as pauseTicketSlaOnPending. A no-op (returning
...
export async function resumeTicketSlaFromPending(
  ...
  const next: TicketSlaApplied = {
    ...applied,
    pausedAt: null,
    timeToResolveDueAt: applied.resolvedAt
      ? applied.timeToResolveDueAt
      : shiftIso(applied.timeToResolveDueAt, shiftMs),
  }

  const [row] = await db
    .update(tickets)
    .set({ slaApplied: next, updatedAt: at })
    .where(ticketSlaStampGuard(ticketId, applied.appliedAt, pausedAt))
    .returning({ id: tickets.id })
  if (!row) return null
