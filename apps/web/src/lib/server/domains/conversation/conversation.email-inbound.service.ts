 ...
 *
 * Of the refusals below, exactly one is retained:
 *
 ...
 *     mail_loop       one of our own outbound mails echoing back. Retaining our
...
export async function ingestParsedEmail(parsed: ParsedInboundEmail): Promise<IngestInboundResult> {
  ...
  if (isAutoGeneratedEmail(parsed, ownDomains)) {
    const loop = isMailLoopEmail(parsed, ownDomains)
    if (
      loop ||
      ...
      logRefusal(loop ? 'mail_loop' : 'auto_generated', false)
...
async function ingestColdInbound(
  ...
  const quarantineCause: InboundRefusalCause | null =
    authSignal === 'sender_auth_reject' ? 'sender_auth_reject' : null
  ...
  try {
    conversationId = await createEmailConversation({
      ...
      // in the open queue. The free-text note is the MTA's own wording, kept
      // as a supplement to the enumerated cause, never as a substitute for it.
      quarantine: quarantineCause
        ? { cause: quarantineCause, note: resolution.verdict.reason }
        : null,
    })
  } catch (error) {
