...
export interface AssistantCandidate {
  ...
  /** True when the turn asked to close the conversation as resolved. */
...
export interface CommitAssistantOutcomeInput {
  ...
  /**
   * The exact passages the generator was given. `internal` rides the input but
   * is not persisted: the stored row records the audience and the provenance,
   * and whether a source is internal is a live property of that source rather
...
export async function commitAssistantOutcome(
  ...
  // The deterministic publication validator, under the fence and always
  // enforced. It runs here rather than after generation because two of its
  // rules are statements about the present: a source can be revoked and a
  // receipt can settle between the candidate being produced and it landing.
  const verdict = await runPublicationValidation(tx, run, input)
  if (!verdict.ok) {
    return { kind: 'rejected', reason: `validation:${verdict.code}` }
  }
  ...
  if (input.evidence?.length) {
    await recordRunEvidence(
      tx,
      input.evidence.map(({ internal: _internal, ...row }) => ({
        ...row,
        runId: run.id,
