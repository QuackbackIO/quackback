...
describe.skipIf(!fixture.available)('ImportUserResolver emailVerified', () => {
  ...
  it('creates a user with emailVerified=true and counts the assertion', async () => {
    ...
    expect(resolver.verifiedCreateCount).toBe(1)
  ...
  it('never flips an existing user and does not count them', async () => {
    ...
    expect(resolver.verifiedCreateCount).toBe(0)
    await resolver.flushPendingCreates()
