/**
 * Moderation service — core moderation logic shared by the server-fn handlers
 * (functions/moderation.ts) and the public REST API (routes/api/v1/moderation/*).
 *
 * - listPendingPosts     — team-only feed of posts in moderationState='pending'
 * - approvePost          — guarded transition: pending → published (ConflictError if not pending)
 * - rejectPost           — guarded soft-delete: sets deletedAt on a pending post with optional
 *                          reason in the audit trail; restoring returns it to the queue.
 * - listPendingComments  — team-only feed of comments in moderationState='pending'
 * - approveComment       — guarded transition + commentCount reconciliation in one transaction
 * - rejectComment        — guarded soft-delete of a pending comment
 *
 * These are plain async functions: auth gating lives in the callers (the server
 * fns gate team-only; the REST routes gate via scopes + permissions). The audit
 * actor + request headers are passed in by the caller so the audit row reflects
 * the real principal regardless of entry point. announce* dispatch failures are
 * swallowed: the entity is already published and audited, and the retry path is
 * blocked by the POST_NOT_PENDING / COMMENT_NOT_PENDING guard, so re-raising
 * would surface a 500 to the moderator and permanently lose webhooks/mentions.
 */
import {
  db,
  posts,
