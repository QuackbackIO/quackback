/**
 * `cp_tenant_schema_state` claiming, against a real Postgres, through the
...
async function seed(tenantId: string, opts?: { maxAttempts?: number; cohort?: string }) {
...
beforeEach(async () => {
  await sql`DELETE FROM cp_tenant_schema_state`
...
describe('claiming', () => {
  it('claims a behind tenant and stamps a lease', async () => {
    await register('t1')
    await seed('t1')
    const claimed = await claimTenants({ limit: 5, leaseMs: 60_000, workerId: 'w1' })
    expect(claimed.map((c) => c.tenantId)).toEqual(['t1'])
    expect(claimed[0]!.leaseToken).toMatch(/^[0-9a-f-]{36}$/)
    expect(claimed[0]!.attempts).toBe(1)

    const [row] = await sql`SELECT status, locked_by FROM cp_tenant_schema_state`
    expect(row!.status).toBe('running')
    expect(row!.locked_by).toBe('w1')
  })

  it('does not claim a tenant already at its target', async () => {
    await register('t1')
    await seed('t1')
    ...
    expect(await claimTenants({ limit: 5, leaseMs: 60_000, workerId: 'w1' })).toEqual([])
  })
  ...
  it('refuses to record success BELOW the target, which would make the row unclaimable', async () => {
    ...
    // rollout reports complete having skipped the tenant.
    await register('t1')
    await seed('t1')
    const [claim] = await claimTenants({ limit: 5, leaseMs: 60_000, workerId: 'w1' })
    expect(
      await completeTenant(claim!, { version: OLD, appliedCount: 226, postconditionsOk: true })
    ).toBe(false)
    const [row] = await sql`SELECT status, current_version FROM cp_tenant_schema_state`
    expect(row!.status).toBe('running')
  ...
  it('the database refuses a succeeded row below its target too', async () => {
    ...
    await seed('t1')
    await expect(
      sql`UPDATE cp_tenant_schema_state
             SET status = 'succeeded', current_version = ${OLD}, postconditions_ok = true,
                 last_verified_at = now()`
    ).rejects.toThrow(/success_evidence/)
  })

  it('re-claims a tenant once its target moves ahead of what it reached', async () => {
    await register('t1')
    await seed('t1')
    const [claim] = await claimTenants({ limit: 5, leaseMs: 60_000, workerId: 'w1' })
    await completeTenant(claim!, { version: TARGET, appliedCount: 228, postconditionsOk: true })
    expect(await claimTenants({ limit: 5, leaseMs: 60_000, workerId: 'w2' })).toEqual([])
    await setTargetVersion({ targetVersion: TARGET + 5000, tenantIds: ['t1'] })
  ...
  it('two concurrent migrators take disjoint tenants', async () => {
    ...
    const [left, right] = await Promise.all([
      claimTenants({ limit: 3, leaseMs: 60_000, workerId: 'w1' }),
      claimTenants({ limit: 3, leaseMs: 60_000, workerId: 'w2' }),
    ])
    const all = [...left, ...right].map((c) => c.tenantId)
  ...
  it('narrows by cohort, which is how a rollout is staged', async () => {
    ...
    await seed('rest')
    const claimed = await claimTenants({
      limit: 5,
      leaseMs: 60_000,
      workerId: 'w1',
      cohort: 'canary',
    })
    expect(claimed.map((c) => c.tenantId)).toEqual(['canary'])
  })
})
...
describe('the fencing token', () => {
  it('refuses a completion from a migrator whose lease was reaped', async () => {
    ...
    const [claim] = await claimTenants({ limit: 1, leaseMs: 60_000, workerId: 'slow' })

    // The reaper takes it back and hands it to someone else.
    await sql`UPDATE cp_tenant_schema_state SET locked_until = now() - interval '1 second'`
    await reapExpiredTenantLeases()
    const [next] = await claimTenants({ limit: 1, leaseMs: 60_000, workerId: 'fast' })
  ...
  it('a heartbeat from a lost lease returns false', async () => {
    ...
    await seed('t1', { maxAttempts: 3 })
    const [claim] = await claimTenants({ limit: 1, leaseMs: 60_000, workerId: 'slow' })
    expect(await heartbeatTenant(claim!, 60_000)).toBe(true)
    await sql`UPDATE cp_tenant_schema_state SET locked_until = now() - interval '1 second'`
    await reapExpiredTenantLeases()
    expect(await heartbeatTenant(claim!, 60_000)).toBe(false)
  })
})

describe('the reaper', () => {
  it('requeues a tenant with attempts left and terminates one without', async () => {
    await register('retry', 'spent')
    await seed('retry', { maxAttempts: 3 })
    await seed('spent', { maxAttempts: 1 })
    await claimTenants({ limit: 5, leaseMs: 60_000, workerId: 'doomed' })
    await sql`UPDATE cp_tenant_schema_state SET locked_until = now() - interval '1 second'`
    ...
    // A poisonous tenant that kills its migrator every time must stop being
    // claimed, or it wakes its Neon compute forever.
    expect(byId.spent!.lastError).toContain('no attempts remaining')
  })

  it('never touches a blocked tenant', async () => {
    await register('t1')
    await seed('t1')
    await sql`UPDATE cp_tenant_schema_state SET status = 'blocked', last_error = 'halted'`
    expect(await reapExpiredTenantLeases()).toEqual({ requeued: 0, terminated: 0 })
...
describe('evidence', () => {
  it('refuses to record success without a verified post-condition verdict', async () => {
    await register('t1')
    await seed('t1')
    const [claim] = await claimTenants({ limit: 1, leaseMs: 60_000, workerId: 'w1' })
    await expect(
      completeTenant(claim!, { version: TARGET, appliedCount: 228, postconditionsOk: false })
    ).rejects.toThrow(/not evidence/)
    const [row] = await sql`SELECT status FROM cp_tenant_schema_state`
    expect(row!.status).toBe('running')
  })
  ...
  it('the database itself refuses a succeeded row with no evidence', async () => {
    ...
    await expect(sql`UPDATE cp_tenant_schema_state SET status = 'succeeded'`).rejects.toThrow(
      /success_evidence/
    )
  })
  ...
  it('the database refuses a terminal failure with no reason', async () => {
    await register('t1')
    await seed('t1')
    await expect(sql`UPDATE cp_tenant_schema_state SET status = 'failed'`).rejects.toThrow(
      /failure_reason/
    )
  })
  ...
  it('records the observation alongside a failure', async () => {
    ...
    const [claim] = await claimTenants({ limit: 1, leaseMs: 60_000, workerId: 'w1' })
    const outcome = await failTenant(claim!, 'postconditions violated: an index is INVALID', {
      appliedCount: 228,
...
describe('intent', () => {
  it('setTargetVersion clears a terminal failure so a rollout can resume', async () => {
    ...
    await seed('t1', { maxAttempts: 1 })
    const [claim] = await claimTenants({ limit: 1, leaseMs: 60_000, workerId: 'w1' })
    await failTenant(claim!, 'boom')
    expect((await listSchemaState())[0]!.status).toBe('failed')

    await setTargetVersion({ targetVersion: TARGET, tenantIds: ['t1'] })
  ...
  it('setTargetVersion does not disturb a tenant that is mid-migration', async () => {
    await register('t1')
    await seed('t1')
    await claimTenants({ limit: 1, leaseMs: 60_000, workerId: 'w1' })
    await setTargetVersion({ targetVersion: TARGET + 5000, tenantIds: ['t1'] })
    const row = (await listSchemaState())[0]!
    expect(row.status).toBe('running')
  ...
  it('ensureSchemaStateRow never lowers an existing target', async () => {
    ...
    await seed('t1')
    await setTargetVersion({ targetVersion: TARGET + 5000, tenantIds: ['t1'] })
    expect(await ensureSchemaStateRow({ tenantId: 't1', targetVersion: OLD })).toBe(false)
