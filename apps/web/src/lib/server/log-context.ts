 ...
 *
 * A single store holds request-scoped identity so the logger can stamp every
 * line (request_id, tenant_id, ...) without passing a logger down the call
 * stack. Open the scope once in the global request middleware; enrich it with
 * setLogContext() once auth/tenant resolution runs.
 *
 * Server-only: imports node:async_hooks. Lives under lib/server/ and must never
 * be imported from client/isomorphic modules.
 */
