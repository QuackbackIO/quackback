 ...
 *
 * The relay is the sole enqueuer onto the `events` queue; every other
 * module must go through the process.ts helpers (which the relay calls, and
 * which enqueue onto the Postgres job queue). This gate fails if any events/
 * module other than the queue owner reintroduces a legacy direct-enqueue
 * construct — the load-bearing guard against a direct-enqueue path that would
 * bypass deterministic dedupe keys and cause duplicate deliveries. (The complementary "no import of the deleted
...
// fileURLToPath then throws at load. Mirrors the structural gates in jobs/__tests__.
const EVENTS_DIR = join(__dirname, '..')

/** process.ts owns the enqueue surface (enqueueHookJobsWithIds/addDelayedJob). */
...
describe('WO-19 enqueue gate', () => {
  it('only the queue owner enqueues onto {event-hooks}', () => {
    ...
    for (const file of walk(EVENTS_DIR)) {
      ...
      // Match legacy direct-enqueue CONSTRUCTS (a queue-client instance / bulk add), not
