...
export async function queueAppHookSync(
  ...
  if (!executor)
    return db.transaction((tx) => queueAppHookSync(provider, queue, deliveryId, payload, tx))
  ...
  if (op?.state === 'queued')
    await enqueueJob({
      queue,
      payload: { operationId: op.id, version: op.version },
      dedupeKey: `sync:${op.id}:${op.version}`,
      maxAttempts: 3,
      executor,
    })
}
