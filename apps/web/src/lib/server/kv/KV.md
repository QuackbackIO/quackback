...
Under `QUACKBACK_TENANCY=pooled` the row is *additionally* in the tenant's own
...
sends a real NOTIFY from a *second* connection and waits for it.
...
listens for *every* tenant. The bound here is *tenants with a live SSE stream on
this replica*, which is already proportional to a long-lived resource.
...
2. **Paired against `SELECT 1` on a real Neon compute**, `aws-us-east-1`, on a
   **throwaway project nobody else was on** (`gauntlet-p8-bench`, created and torn
   down for the measurement — a shared tenant compute has other pieces' pollers on
...
that is **not** what a Neon tenant pays. On the dedicated Neon compute, paired
...
per *rate-checked* request, so it lands on sign-in and on the widget's
...
is also small: S live streams cost S/20 writes per second per tenant, so 100
concurrent agent streams is **5 writes/second**.

The genuinely new cost is on the *disconnect* path, not the heartbeat: a
