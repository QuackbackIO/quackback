...
describe('deriveTenantSecret', () => {
  it('matches its pinned vector', () => {
    ...
    expect(
      deriveTenantSecret(ROOT, { generation: 1, tenantId: 'inst_alpha', purpose: 'app-secrets' })
    ).toBe('iUPXg6l7NqiwxuiTBPtKxvJnhOPsbqW-ttMXtxvnlUg')
  })

  it('gives two tenants different keys from one root', () => {
    const alpha = deriveTenantSecret(ROOT, {
      generation: 1,
      tenantId: 'inst_alpha',
      purpose: 'app-secrets',
    })
    const bravo = deriveTenantSecret(ROOT, {
      generation: 1,
      tenantId: 'inst_bravo',
      purpose: 'app-secrets',
    })
  ...
  it('gives one tenant different keys per generation, so rotation is expressible', () => {
    const g1 = deriveTenantSecret(ROOT, {
      generation: 1,
      tenantId: 'inst_alpha',
      purpose: 'app-secrets',
    })
    const g2 = deriveTenantSecret(ROOT, {
      generation: 2,
      tenantId: 'inst_alpha',
      purpose: 'app-secrets',
    })
  ...
  it('refuses a root shorter than the floor', () => {
    ...
    expect(() =>
      deriveTenantSecret('short', { generation: 1, tenantId: 't', purpose: 'app-secrets' })
    ).toThrow(FleetSecretError)
    expect(FLEET_ROOT_KEY_MIN_LENGTH).toBe(32)
  })
})

describe('sealTenantSecret / openTenantSecret', () => {
  const target = { generation: 1, tenantId: 'inst_alpha', purpose: 'storage' } as const

  it('round-trips', () => {
    const blob = sealTenantSecret(ROOT, target, '{"accessKeyId":"AK","secretAccessKey":"SK"}')
    expect(openTenantSecret(ROOT, target, blob)).toBe(
      '{"accessKeyId":"AK","secretAccessKey":"SK"}'
    )
  })

  it('produces a different blob every time, so a ref is not a fingerprint', () => {
    const a = sealTenantSecret(ROOT, target, 'x')
    const b = sealTenantSecret(ROOT, target, 'x')
    expect(a).not.toBe(b)
    expect(openTenantSecret(ROOT, target, a)).toBe('x')
  })

  it('refuses a blob moved to another tenant', () => {
    // The gate that makes a sealed ref safe to carry in a row: a record that
    // named another tenant's blob would otherwise hand this process that
    // tenant's bucket credentials.
    const blob = sealTenantSecret(ROOT, target, 'secret')
    expect(() =>
      openTenantSecret(ROOT, { ...target, tenantId: 'inst_bravo' }, blob)
    ).toThrow(/did not open/)
  })

  it('refuses a blob from another generation', () => {
    const blob = sealTenantSecret(ROOT, target, 'secret')
    expect(() => openTenantSecret(ROOT, { ...target, generation: 2 }, blob)).toThrow(
      /did not open/
    )
  })
  ...
  it('refuses a blob under a different root', () => {
    const blob = sealTenantSecret(ROOT, target, 'secret')
    expect(() => openTenantSecret(OTHER_ROOT, target, blob)).toThrow(/did not open/)
  })
...
describe('the SECRET_KEY canary', () => {
  it('opens under the key it was sealed with', () => {
    const key = deriveTenantSecret(ROOT, {
      generation: 1,
      tenantId: 'inst_alpha',
      purpose: 'app-secrets',
    })
  ...
  it('does NOT open under another tenant’s key — the case it exists for', () => {
    const alphaKey = deriveTenantSecret(ROOT, {
      generation: 1,
      tenantId: 'inst_alpha',
      purpose: 'app-secrets',
    })
    const bravoKey = deriveTenantSecret(ROOT, {
      generation: 1,
      tenantId: 'inst_bravo',
      purpose: 'app-secrets',
    })
  ...
  it('does not open under a canary sealed for a different tenant id', () => {
    const key = deriveTenantSecret(ROOT, {
      generation: 1,
      tenantId: 'inst_alpha',
      purpose: 'app-secrets',
  ...
  it('does not open under a root rotation the record has not caught up with', () => {
    const oldKey = deriveTenantSecret(ROOT, {
      generation: 1,
      tenantId: 'inst_alpha',
      purpose: 'app-secrets',
    })
    const newKey = deriveTenantSecret(OTHER_ROOT, {
      generation: 1,
      tenantId: 'inst_alpha',
      purpose: 'app-secrets',
    })
    expect(verifySecretKeyCanary(newKey, 'inst_alpha', sealSecretKeyCanary(oldKey, 'inst_alpha'))).toBe(
      false
    )
  })
