/**
 * Cache headers for responses whose body depends on which tenant asked.
 *
 * Under pooled tenancy the `Host` header chooses the database, so a publicly
 * cacheable response is per-tenant content on a URL path that every tenant
 ...
 * through the edge rather than through the pool: tenant A's branding, widget
 * configuration and asset served to tenant B, with nothing erroring and nothing
 ...
 */
export function publicTenantCacheHeaders(
  maxAge: number,
  ...alsoVaryOn: string[]
