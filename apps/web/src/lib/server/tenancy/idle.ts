 ...
 * > tenant database once per second forever, so the compute never suspends
 * > whatever this cache does.
 *
 * Measured on a four-tenant fleet, that is exactly what happened: two `LISTEN`
 * connections per tenant held for 14h33m, four `postgres.js` connections
 * alongside them, and every tenant database at 45–70% active while doing no work
 * at all. A database nothing connects to sat at 2%. The platform suspends
 * correctly; the tiers were the reason it could not.
 *
 * ## The policy
 *
 * A tier holds its connections while a tenant is *doing something*, and lets go
 ...
 * deliberately — a detached tenant is one whose doorbell is *known* to be
 ...
 *
 * 1. **A signal.** Something in this process opened a tenant scope for a reason
 ...
 *    opening that scope already connected to the tenant database and therefore
 ...
 * a tenant scope this process opened. Under a split `role=web` + `role=worker`
 ...
 * costs nothing per tenant and reaches every replica. But a permanent `LISTEN`
 * is a permanently-connected client, and the control database is now required to
 * suspend when the fleet goes quiet for exactly the same reason the tenants are.
 ...
 * the only thing this number buys is not thrashing: a tenant that goes quiet for
 ...
 * the 1s poll interval, so a tenant doing anything at all at even 1/60 of the
 * poll cadence never detaches. It deliberately sits *above*
 * `tenantPoolIdleSeconds` (45s): the request cache should let go first, so that
 * by the time a tier detaches there is nothing of that tenant's left here.
 *
 ...
 * afterwards: a *fully* idle tenant is awake for roughly
 ...
 * not lost work, and fifteen minutes of staleness on a tenant with literally no
 ...
 * always-warm behaviour back, which is the correct setting for a single-tenant
 * install on a database that is never billed for idleness.
 */
import { logger } from '@/lib/server/logger'

const log = logger.child({ component: 'tenant-idle' })

export interface TenantIdlePolicy {
  /** Silence before a tier releases every connection it holds for a tenant. */
...
export function tenantIdlePolicy(): TenantIdlePolicy {
  return {
...
export function idleDetachDisabled(policy: TenantIdlePolicy): boolean {
...
/** Who told us a tenant is busy. */
export type TenantActivitySource = 'request' | 'sweep' | 'script' | 'migration'

type Subscriber = (tenantId: string, source: TenantActivitySource) => void

const subscribers = new Set<Subscriber>()

/**
 * Say that something outside the always-warm tiers is using this tenant.
 *
 ...
 * to remove — so the call site is `acquireTenantScope`, filtered by scope
 * origin, and `queue` and `relay` are excluded there.
 */
export function noteTenantActivity(tenantId: string, source: TenantActivitySource): void {
  ...
  for (const fn of subscribers) {
    try {
      ...
      log.warn({ err, tenantId, source }, 'tenant activity subscriber threw')
    }
  }
}

/** Subscribe to the signal. Returns an unsubscribe function. */
export function onTenantActivity(fn: Subscriber): () => void {
  subscribers.add(fn)
  return () => {
...
export function __resetTenantActivityForTests(): void {
  subscribers.clear()
}
