 ...
 * the tenant database: `settings.id` and the control plane's stamp. Both are
 * data, and Neon branching copies data — so a branch of a tenant's database
 * satisfies both halves and is served as the real thing:
 *
 *   REAL   ws=019fde94-…  stamp tenantId=inst_gauntlet_alpha   verdict {"ok":true}
 *   BRANCH ws=019fde94-…  stamp tenantId=inst_gauntlet_alpha   verdict {"ok":true}
 *
...
/** Where the registry says the tenant physically lives. */
export type PhysicalExpectation = {
  /** Neon project id, or null when the tenant is not on Neon. */
  neonProjectId: string | null
  /** Neon branch id (`br-…`), or null when the tenant is not on Neon. */
 ...
 * A record claiming no Neon placement (a self-hosted tenant) skips the check
...
export function evaluatePhysicalIdentity(
  ...
  if (observed.neonProjectId === null && observed.neonBranchId === null) {
    return {
      ...
      detail:
        'registry places this tenant on Neon but the connected database reports no ' +
  ...
  if (expected.neonBranchId !== null && observed.neonBranchId !== expected.neonBranchId) {
    return {
      ...
      detail:
        ...
        'this is a branch of the tenant database, not the tenant database. Branching copies both ' +
