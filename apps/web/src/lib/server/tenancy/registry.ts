 ...
 *
 * Tiny and long-lived on purpose: it is one shared read path for the whole
 * instance, it is the only database a pooled process may touch without a tenant
 * scope, and it must never be confused with a tenant pool. `idle_timeout` is
 * deliberately generous — unlike a tenant pool this one is not part of the
 * scale-to-zero story, and reconnect churn on the request path is worse than a
 * held socket to a control database that is always warm anyway.
 */
export function getControlSql(): postgres.Sql {
  ...
  if (!url) {
    throw new Error(
      'QUACKBACK_CONTROL_DATABASE_URL is not set; the tenant registry cannot be read'
    )
  }
  controlSql = postgres(url, { max: 2, idle_timeout: 120, connect_timeout: 10, onnotice: () => {} })
  return controlSql
}
...
export async function listActiveTenants(
  sql: postgres.Sql = getControlSql()
): Promise<{ tenants: TenantDescriptor[]; refused: Array<{ tenantId: string; problems: string[] }> }> {
  const rows = (await sql.unsafe(
