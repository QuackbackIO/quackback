/**
 * Per-tenant application secrets, derived or sealed under one fleet root.
 *
 ...
 * fleet root with the tenant id as domain separation and every replica computes
 ...
 * already on the database credential (`docs/neon-tenant-provisioning.md` §5).
 *
 ...
 * bound to the tenant, and the sealed blob rides in the reference itself — which
 ...
 *
 * One root key opens every tenant. That is worth saying plainly rather than
 * burying — but it is not a regression: **today every pooled tenant shares one
 ...
 * re-encrypt: a tenant whose ref stops saying `derived+hkdf://` and starts
 * saying something else is a per-tenant decision, one record at a time.
 *
 * ## Rotation
 *
 * Every ref carries a **generation**. A new root is installed alongside the old
 * one; refs are rewritten to the new generation tenant by tenant; the old
 * generation stays derivable throughout, so re-encrypting a tenant's ciphertext
...
export interface FleetSecretTarget {
  ...
  generation: number
  /** The tenant this secret belongs to. Bound into the key, not just the path. */
  tenantId: string
...
function assertTarget(target: FleetSecretTarget): void {
  ...
  if (typeof target.tenantId !== 'string' || target.tenantId === '') {
    throw new FleetSecretError('bad_tenant', 'a fleet secret target needs a tenant id')
  }
...
/** `quackback:fleet:<kind>:v<gen>:<tenantId>:<purpose>` — the HKDF info string. */
function info(kind: 'derive' | 'seal', target: FleetSecretTarget): string {
  return `quackback:fleet:${kind}:v${target.generation}:${target.tenantId}:${target.purpose}`
}
...
/**
 * The per-tenant secret for `target`, as URL-safe base64.
 *
 ...
 */
export function deriveTenantSecret(rootKey: string, target: FleetSecretTarget): string {
...
/**
 * Seal `plaintext` for one tenant and purpose.
 *
 * The tenant and purpose are bound twice — once through the key's HKDF info, and
 ...
 */
export function sealTenantSecret(
  rootKey: string,
 ...
 * Open a sealed blob.
 *
 * Every failure is one error with one code. A blob sealed for another tenant, a
 ...
 */
export function openTenantSecret(rootKey: string, target: FleetSecretTarget, blob: string): string {
  assertRootKey(rootKey)
  assertTarget(target)
  let raw: Buffer
  try {
  ...
  try {
    ...
    throw new FleetSecretError(
      'sealed_unopenable',
      `sealed secret for ${target.tenantId}/${target.purpose} (generation ${target.generation}) ` +
        'did not open: wrong root key, wrong generation, or a blob sealed for another tenant'
    )
  }
}

function aad(target: FleetSecretTarget): string {
  return `v${target.generation}|${target.tenantId}|${target.purpose}`
}
 ...
 * fleet holding the wrong key for a tenant writes *new* ciphertext under it
 ...
 *
 * So the key gets the same treatment §3 gives the database: **the tenant
 ...
 * refuses to serve the tenant instead of writing under a key that will not open
 * tomorrow.
 *
 ...
 */
export function sealSecretKeyCanary(secretKey: string, tenantId: string): string {
  ...
  cipher.setAAD(Buffer.from(`canary|${tenantId}`, 'utf8'))
...
export function verifySecretKeyCanary(
  ...
  try {
    ...
    decipher.setAAD(Buffer.from(`canary|${tenantId}`, 'utf8'))
