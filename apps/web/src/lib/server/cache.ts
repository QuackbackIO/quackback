 ...
 *
 * The edge: under pooled tenancy `db` throws `TenantScopeMissingError` when
 * there is no tenant scope, where Redis would have quietly used the `_`
 ...
 * correct direction — a miss is slow, a shared namespace is one tenant's
 ...
 *
 * These are the strings that reach the `key` column verbatim. The tenant
 * discriminator is NOT part of them — it is `kv_store.tenant_id`, the leading
 ...
 * always one `${'settings:tenant'}:extra` away from being bypassed. A key
 * column cannot be bypassed by concatenation; the worst a malformed key can do
 * is collide with another key of the same tenant.
 */
