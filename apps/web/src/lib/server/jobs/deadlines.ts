 ...
 *
 * Two schedules in `definitions.ts` ran on `* * * * *` for every tenant, for
 * ever: `snooze-sweep` and `sla-breach-sweep`. That is 2,880 job executions per
 * tenant per day, and — on a platform that suspends a compute after about five
 ...
 * extreme: a five-minute sweep wakes every tenant precisely as it is about to
 ...
 * it. So the tenant can be asked "when is your next deadline?" instead of being
 * woken to be asked "is anything due?".
 *
 * That removes the trade rather than repricing it:
 *
 * - a tenant with nothing pending contributes **no wakes at all** — the schedule
 *   is inert, the tiers detach, and the compute suspends;
 * - a tenant with a deadline three days out is inert for three days and wakes
 *   once, at the deadline;
 * - a tenant with a deadline in the next minute ticks exactly as it does today,
 ...
 * 2. `earliestTenantDeadline()` tells a **detaching** tier when to come back, so
 *    a tenant that has gone quiet with a deadline pending still wakes for it
...
/**
 * The earliest deadline across every registered queue for this tenant, rounded
 ...
 */
export async function earliestTenantDeadline(now = Date.now()): Promise<Date | null> {
