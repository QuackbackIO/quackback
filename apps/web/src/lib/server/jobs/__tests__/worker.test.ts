...
describe('pooled job worker', () => {
  ...
  it('warns when durable Quinn has no local worker or worker URL', async () => {
    vi.resetModules()
    vi.doMock('@/lib/server/process-role', () => ({ shouldRunWorkers: () => false }))
    vi.stubEnv('ASSISTANT_EXECUTION_MODE', 'durable')
    vi.stubEnv('QUACKBACK_JOB_WORKER_URL', '')
    const { logger } = await import('@/lib/server/logger')
    const warn = vi.spyOn(logger, 'warn')
    const mod = await import('../worker')
    await mod.startJobWorker()
    expect(warn.mock.calls.flat().join(' ')).toContain('QUACKBACK_JOB_WORKER_URL')
    warn.mockClear()
    vi.stubEnv('QUACKBACK_JOB_WORKER_URL', 'http://worker:3000')
    await mod.startJobWorker()
