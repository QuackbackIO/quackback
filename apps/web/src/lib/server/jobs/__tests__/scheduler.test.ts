...
describe('tenant scheduler', () => {
  ...
  it('coalesces repeated signals into one in-flight run', async () => {
    ...
    const scheduler = createTenantScheduler({
      clock,
      runTenant: async () => {
        started += 1
        await gate
  ...
  it('removes a tenant with no deadline from the heap', async () => {
    const clock = fakeClock()
    const scheduler = createTenantScheduler({
      clock,
      runTenant: async () => null,
    })
    scheduler.signal('ws_a')
    await scheduler.idle()
    expect(scheduler.size()).toBe(0)
  ...
  it('stores only the next wake time after a pass', async () => {
    ...
    const scheduler = createTenantScheduler({
      clock,
      runTenant: async () => new Date(clock.now() + 30_000),
    })
    scheduler.signal('ws_a')
    await scheduler.idle()
  ...
  it('one bad tenant does not block another', async () => {
    const clock = fakeClock()
    const ran: string[] = []
    const scheduler = createTenantScheduler({
      clock,
    ...
    expect(scheduler.peek()?.tenantId).toBe('ws_bad')
    scheduler.stop()
  })
