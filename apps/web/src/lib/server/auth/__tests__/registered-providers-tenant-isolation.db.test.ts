 ...
 * between tenants, so a per-tenant fake store would assume away the thing under
 * test. One table for both tenants — under pooled tenancy a tenant additionally
 * has its own database, so this is the weaker of the two arrangements and the
 * right one to assert against. The separation has to come from `tenant_id`.
 *
...
vi.mock('@/lib/server/domains/settings/settings.service', () => ({
  ...
  getTenantSettings: async () => {
    hoisted.computes.push(hoisted.currentTenantId())
...
const [ALPHA_ID, BRAVO_ID] = tenantPair()
...
afterAll(async () => {
  await cleanupTenants(ALPHA_ID, BRAVO_ID)
  await closeHarness()
})

beforeEach(async () => {
  // Both tenants keep their ids across the file, so the cached rows have to go
...
describe('the fixture reaches the code under test', () => {
  ...
  it('alpha computes its own provider ids', async () => {
    expect(await withRealTenant(ALPHA_ID, () => getRegisteredAuthProviders())).toEqual([
      'alpha-workforce-idp',
      'google',
    ])
  })
  ...
  it('bravo computes its own provider ids', async () => {
    expect(await withRealTenant(BRAVO_ID, () => getRegisteredAuthProviders())).toEqual([
      'bravo-partner-idp',
    ])
  })

  it('the cache is really a cache — the second call does not recompute', async () => {
    await withRealTenant(ALPHA_ID, () => getRegisteredAuthProviders())
...
describe('REGISTERED_AUTH_PROVIDERS does not cross tenants', () => {
  it('bravo is not offered alpha’s OIDC provider when alpha primed the cache', async () => {
    await withRealTenant(ALPHA_ID, () => getRegisteredAuthProviders())
    const bravo = await withRealTenant(BRAVO_ID, () => getRegisteredAuthProviders())
  ...
  it('alpha is not offered bravo’s OIDC provider when bravo primed the cache', async () => {
    ...
    // on which tenant's value happened to survive a shared store, so a
    // one-directional check here would be the same defect in a new place.
    await withRealTenant(BRAVO_ID, () => getRegisteredAuthProviders())
    const alpha = await withRealTenant(ALPHA_ID, () => getRegisteredAuthProviders())

    expect(alpha).not.toContain('bravo-partner-idp')
    expect(alpha).toEqual(['alpha-workforce-idp', 'google'])
  })

  it('writes two distinct rows for the same logical key, one per tenant', async () => {
    await withRealTenant(ALPHA_ID, () => getRegisteredAuthProviders())
    await withRealTenant(BRAVO_ID, () => getRegisteredAuthProviders())

    // The successor of "two distinct Redis keys": the logical key is byte-for-
    // byte the same on both rows, and `tenant_id` is the only thing keeping the
    // second write from overwriting the first. Scoped to this file's two tenant
    // ids — `quackback_test` is shared with every other checkout on the machine.
    const rows = await testSql()<{ tenant_id: string; key: string; value: string[] }[]>`
      SELECT tenant_id, key, value
      FROM kv_store
      WHERE key = ${CACHE_KEYS.REGISTERED_AUTH_PROVIDERS}
        AND tenant_id IN (${ALPHA_ID}, ${BRAVO_ID})
      ORDER BY tenant_id = ${ALPHA_ID} DESC
    `

    expect(rows).toHaveLength(2)
    expect(rows.map((r) => r.tenant_id)).toEqual([ALPHA_ID, BRAVO_ID])
  ...
  it('survives the cache: a value written under one tenant is unreachable from the other', async () => {
    ...
    // store, nothing recomputed it for bravo. A read that missed `tenant_id`
...
describe('the shared OIDC gate underneath it', () => {
  ...
  it('gates on the ACTIVE tenant’s tier flag, not a neighbour’s', async () => {
    ...
    expect([...(await withRealTenant(ALPHA_ID, () => getRegisteredOidcProviderIds()))]).toEqual([
      'alpha-workforce-idp',
    ])
    expect([...(await withRealTenant(BRAVO_ID, () => getRegisteredOidcProviderIds()))]).toEqual([])
  })
})
