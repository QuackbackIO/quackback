...
export async function runPreflight(
  ...
  const makeHandle = (slot: TenantSlot, baseUrl: string): TenantHandle => ({
    slot,
    baseUrl,
  ...
  const both: TenantHandle[] = [alpha, bravo]
  ...
  const dbUrls: Record<TenantSlot, string | undefined> = {
  ...
  const identity: Record<TenantSlot, { workspaceId?: string; slug?: string }> = {
    alpha: {},
    bravo: {},
  }
  for (const handle of both) {
  ...
  if (identity.alpha.workspaceId && identity.bravo.workspaceId) {
    ...
    steps.push({
      ...
      detail: distinct
        ...
        : `BOTH TARGETS SHARE workspace id ${identity.alpha.workspaceId} — this is one tenant, not two`,
    })
    if (!distinct) {
      return finish(
        steps,
        ...
        alpha,
        bravo,
        'alpha and bravo resolve to the same workspace; a single tenant cannot leak to itself and every verdict would be meaningless'
      )
    }
  } else {
    steps.push({
  ...
  for (const [capability, kind] of [
    ...
    if (a && b) {
      ...
      // Two tenants sharing a secret is itself the §4 hazard. Record it here so
      ...
      steps.push({
        ...
        detail:
          a !== b
            ? 'supplied for both tenants and distinct'
            : 'SUPPLIED BUT IDENTICAL for both tenants — a shared secret is a cross-tenant capability',
      })
    } else {
      steps.push({
        name: `credential:${capability}`,
  ...
  if (!capabilities.has('widget-secret') && capabilities.has('db')) {
    const secrets: Partial<Record<TenantSlot, string>> = {}
    for (const handle of both) {
      ...
      steps.push({
        ...
        detail:
          secrets.alpha !== secrets.bravo
            ? 'read from settings.widget_secret on both tenants'
            : 'read from the database and IDENTICAL across tenants — a shared widget secret forges identities in both',
      })
    }
  }
  ...
  if (!capabilities.has('api-key')) {
    steps.push({
      name: 'fixture',
      ok: false,
      detail: 'cannot provision the colliding fixture without REST API keys for both tenants',
    })
    return finish(
      steps,
      capabilities,
      alpha,
      bravo,
      'the colliding two-tenant fixture could not be provisioned (no REST API keys). ' +
