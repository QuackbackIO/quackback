/**
 * Which strings are allowed to accuse a tenant.
 *
 * A marker only means something if its appearance in the other tenant's output
 ...
 * the other tenant's *settings row* also contained it. That let `#ffffff`, a
 * workspace named `Support`, and — worst — the board title this suite writes
 * into BOTH tenants itself, all become accusations. Three correctly isolated
 * fleets exited 2.
 *
 ...
 *    tenant's output on its own is never admitted: achromatic or near-universal
 ...
 * shows the other tenant's identity INSTEAD of its own, which no amount of
...
/**
 * Product vocabulary that appears in ordinary UI chrome regardless of tenant.
 *
...
/** Strings this suite writes into BOTH tenants, so they can never distinguish them. */
...
/**
 * True when a token could turn up in any tenant's output on its own merits, and
 * therefore must never be treated as one tenant's identity.
 */
export function isGenericToken(raw: string): boolean {
  const token = raw.trim()
  if (token.length === 0) return true

  // The fixture writes these into both tenants; finding one on the "wrong"
 ...
 * leaves (`paragraph`, `public`, `none`, layout keys) carry no tenant identity
