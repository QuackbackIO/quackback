#!/usr/bin/env bun
/**
 * Adversarial two-tenant isolation probe suite — entry point.
 *
 * Usage: bun apps/web/tenancy-probe/cli.ts --alpha <url> --bravo <url> [options]
 *        (see --help, or tenancy-probe/README.md)
 *
 * Output contract:
 *   stdout — the JSON report, and nothing else, unless --json-out is given
 *   stderr — the human summary and progress logging
 *   exit   — 0 all passed · 1 a probe could not execute · 2 a cross-tenant observation
 *
...
import type { ProbeLogger, TenantHandle, TenantSlot } from './types'
...
const pino = createLogger({ base: { service_name: 'quackback-tenancy-probe' } })
