...
import type { ProbeReport, TenantHandle } from '../types'
...
describe('exitCodeFor', () => {
  ...
  it('returns 2 for a cross-tenant observation, distinct from a harness failure', () => {
    expect(
...
describe('parseConfig', () => {
  ...
  it('refuses to run with both slots pointed at one origin', () => {
    // A single tenant is trivially consistent with itself, so this
...
describe('verifyCollisions', () => {
  const handle = (slot: 'alpha' | 'bravo', over: Record<string, string> = {}): TenantHandle => ({
    slot,
  ...
  it('rejects identical ids across tenants — that is one database, not two', () => {
