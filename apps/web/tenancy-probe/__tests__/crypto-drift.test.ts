 ...
 * not, every cross-tenant attempt is refused because the token is malformed —
...
describe('storage read capability', () => {
  const secret = 'a-tenant-storage-secret'
  ...
  it('is rejected under a different secret — the whole basis of P03', () => {
    const otherTenantSig = mintStorageReadSig('a-different-tenant-secret', key)
    expect(verifyStorageReadToken(secret, key, otherTenantSig)).toBe(false)
  })

  it('is bound to the key, so P03 must hold the key constant across tenants', () => {
