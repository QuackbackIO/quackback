...
const USAGE = `
quackback tenant-isolation probe

  bun apps/web/tenancy-probe/cli.ts --alpha <url> --bravo <url> [options]

Required
  --alpha <url>              Base URL of the alpha tenant (e.g. http://alpha.localhost:3000)
  --bravo <url>              Base URL of the bravo tenant
...
Fixture identity (must be IDENTICAL for both tenants — the collision is the point)
...
Output and behaviour
  ...
  --teardown                 Remove the probe fixtures from both tenants and exit
  --help                     This text
`.trim()
...
export function parseConfig(argv: string[], env: Record<string, string | undefined>): ProbeConfig {
  ...
  // same tenant answering twice is trivially consistent with itself. Preflight
  ...
  if (alphaUrl === bravoUrl) {
    throw new ConfigError(
      `--alpha and --bravo resolve to the same origin (${alphaUrl}). ` +
        'Two distinct tenants are required; a single deployment cannot leak to itself.'
    )
  }
