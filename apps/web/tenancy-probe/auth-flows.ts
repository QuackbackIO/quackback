...
import type { TenantDb, TenantHandle, TenantHttp } from './types'
...
/** Ask a tenant to mint a magic link, then read the token out of its database. */
...
export async function mintAndReadMagicLink(
  ...
  if (!handle.db) return { sendStatus: 0, detail: 'no database connection for this tenant' }
...
/** Ask a tenant to mint a sign-in OTP, then read the code out of its database. */
