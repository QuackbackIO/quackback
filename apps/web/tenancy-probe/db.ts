 ...
 * connection to each tenant's database:
 *
 ...
 *    database;
 *  - P09, to read each tenant's assistant service-principal id and to check
 *    that no row in one tenant references the other's.
 *
...
import type { TenantDb, TenantSlot } from './types'

/** Thrown when a tenant database cannot be reached or queried. */
...
export function createTenantDb(slot: TenantSlot, connectionString: string): TenantDb {
 ...
 * database has always been the tenant boundary (SAAS-HOSTING-STACK.md §3), so
