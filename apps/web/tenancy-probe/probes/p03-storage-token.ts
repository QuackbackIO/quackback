 ...
 *
 * Under pooled tenancy the signed message is `tenantBind('read|<key>')`
 * (`storage/s3.ts`), which prefixes `t:<tenantId>|` — so alpha's capability
 * carries alpha's tenant id and bravo verifies against bravo's. Once tenant ids
 ...
 * be refused by arithmetic, so the key is held constant. The tenant binding is
 ...
 * the probe mints under each candidate message and keeps the one its own tenant
 * accepts. A `--alpha-tenant-id` that does not match the deployment then fails
...
const STORAGE_LEAK_REASON =
  'a private-object read capability, or the secret that mints one, crossed the tenant boundary. ' +
  'Every private upload, export and attachment URL in one tenant is readable from the other.'
 ...
 *
 * `tenantId` present reproduces the pooled binding (`t:<id>|read|<key>`);
 * absent reproduces the single-tenant message byte for byte. Which one a
...
function candidateBindings(tenantId: string | undefined): Binding[] {
  const unbound: Binding = { label: 'unbound (single-tenant message)' }
  return tenantId ? [{ tenantId, label: `bound to tenant ${tenantId}` }, unbound] : [unbound]
}
...
export const p03StorageToken: Probe = {
  ...
  async run(ctx: ProbeContext) {
    ...
    //
    // Two tenants sharing a tenant id would sign the identical message, which
    // makes the binding no separation at all.
    if (config.alphaTenantId && config.bravoTenantId) {
      const idsDiffer = config.alphaTenantId !== config.bravoTenantId
      controls.push(
        control(
          'invariant',
          'alpha and bravo bind their read capabilities to different tenant ids',
          idsDiffer,
          idsDiffer
            ? `alpha ${config.alphaTenantId}, bravo ${config.bravoTenantId}`
            : `IDENTICAL (${config.alphaTenantId}) — both tenants sign the same message, so the ` +
    ...
    // tenancy the message carries `t:<tenantId>|`; a single-tenant deployment
    ...
    const calibrate = async (
      ...
      for (const binding of candidateBindings(tenantId)) {
        const res = await handle.http.request(
          path(mintStorageReadSig(secret, PRIVATE_KEY, binding.tenantId))
        )
        last = res
    ...
    const calibrated = {
      alpha: await calibrate(alpha, alphaSecret, config.alphaTenantId),
      bravo: await calibrate(bravo, bravoSecret, config.bravoTenantId),
    }
        ...
        control(
          ...
          binding
            ...
            : `REFUSED with 403 under every candidate message (${candidateBindings(
                slot === 'alpha' ? config.alphaTenantId : config.bravoTenantId
              )
                .map((b) => b.label)
                .join(', ')}). ${slot} does not accept a capability minted with the secret supplied ` +
                `for ${slot}: the --${slot}-storage-secret value, or the --${slot}-tenant-id binding, ` +
    ...
    const bindingsDiffer = binding.alpha.tenantId !== binding.bravo.tenantId
    ...
    for (const [fromSlot, to] of [
      ...
      // 1. The faithful replay: the capability exactly as this tenant's own
      //    URLs carry it, presented to the other host.
      const replay = await to.http.request(
        path(mintStorageReadSig(fromSecret, PRIVATE_KEY, fromBinding.tenantId)),
      ...
      //    is accepted if and only if the two tenants' secrets interchange.
      const interchange = await to.http.request(
        path(mintStorageReadSig(fromSecret, PRIVATE_KEY, toBinding.tenantId)),
        { expectsForeignMarkers: true }
      )
      controls.push(
        control(
          ...
          accepted(interchange)
            ? `ACCEPTED (${describeResponse(interchange, 160)}) — ${fromSlot}'s storage secret verifies ` +
              `on ${to.slot}. The two tenants share a storage secret, so anyone who can mint a read ` +
    ...
    return decide({
      ...
      onPass: {
        observed:
          ...
          "tenant's secret even against a message it would otherwise have verified" +
          (bindingsDiffer
            ? ` (the hosts bind different tenant ids, so the plain replay was over-determined and the ` +
              `secret-interchange attempt is what carried the verdict)`
            : ''),
        reason:
          'the read capability is bound to a per-tenant secret, so neither the capability nor the ' +
          'secret that mints it transfers between tenants',
      },
      evidence,
    })
  },
