 ...
 *
 * The probe drives a real write on each tenant through the REST API, lets the
 * derived background work settle, and then asks a question that needs no
 * knowledge of which queue ran: does anything anywhere in the other tenant's
...
import type { ControlOutcome, Probe, ProbeContext, TenantHandle, TenantSlot } from '../types'
 ...
 * tenant's database gained derived rows referencing its post" — was satisfied
 ...
 * tenant database's `now()` is exactly the skew-sensitive comparison a probe
 ...
 */
function mintDriveToken(slot: TenantSlot): string {
  return `qbprobedrive${slot}${randomBytes(6).toString('hex')}`
}

function driveContent(token: string): string {
  return `Tenant isolation probe drive ${token}. Safe to delete.`
}
...
export const p07BackgroundJob: Probe = {
  ...
  poolingCaveat:
    ...
    'per-tenant DATABASE_URL, so a misrouted job CAN reach the other database and this probe is ' +
    'load-bearing today. What remains over-determined is narrower: the probe observes rows, not ' +
    'processes, so it cannot tell "the worker resolved the right tenant" from "the worker resolved ' +
    'the wrong one and the write happened to fail" — nor can it see a job that ran against the ' +
    'wrong tenant and only READ. A clean result here is evidence about writes reaching the wrong ' +
    'database, and about nothing else the worker tier does.',

  async run(ctx: ProbeContext) {
    const { alpha, bravo, config } = ctx
    const attempted =
      "post a comment carrying a freshly minted per-run drive token to each tenant's fixture post " +
      'through the REST API, wait for the derived background work to settle, then scan each ' +
      "tenant's entire content, event, conversation and job schema for any reference to the other " +
      'tenant’s post or drive token'
    ...
    //
    // Both tenants are driven before the settle, so the symmetric check costs one
    // settle window rather than two. Each run leaves one comment per tenant;
      ...
      if (derived.length === 0) {
        ...
        return halt({
          ...
          reason:
            ...
            'the settle window, so this probe is blind: an absence of rows in the other tenant ' +
    ...
    return decide({
      ...
      onPass: {
        observed:
          `both tenants gained derived rows carrying this run's drive token ` +
