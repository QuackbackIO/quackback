...
export function useCommentPermissions({ commentId, enabled = true }: UseCommentPermissionsOptions) {
  return useQuery({
    ...
    queryFn: async (): Promise<CommentPermissions> => {
      const result = await getCommentPermissionsAction({ commentId })
      if (!result.success) {
...
export function useEditComment({ commentId, postId, onSuccess, onError }: UseEditCommentOptions) {
  ...
  return useMutation({
    mutationFn: async (content: string) => {
      const result = await userEditCommentAction({ commentId, content })
      if (!result.success) {
        throw result.error
      }
      return result.data
...
export function useDeleteComment({
  ...
  return useMutation({
    mutationFn: async () => {
      const result = await userDeleteCommentAction({ commentId })
      if (!result.success) {
        throw result.error
      }
      return result.data
