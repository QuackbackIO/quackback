...
export function useSubscribe({ postId, onSuccess, onError }: UseSubscribeOptions) {
  ...
  return useMutation({
    mutationFn: async (reason: 'manual' | 'author' | 'vote' | 'comment' = 'manual') => {
      const result = await subscribeToPostAction({ postId, reason })
      if (!result.success) {
        throw result.error
      }
      return result.data
...
export function useUnsubscribe({ postId, onSuccess, onError }: UseUnsubscribeOptions) {
  ...
  return useMutation({
    mutationFn: async () => {
      const result = await unsubscribeFromPostAction({ postId })
      if (!result.success) {
        throw result.error
      }
      return result.data
...
export function useMuteSubscription({ postId, onSuccess, onError }: UseMuteSubscriptionOptions) {
  ...
  return useMutation({
    mutationFn: async (muted: boolean) => {
      const result = await muteSubscriptionAction({ postId, muted })
      if (!result.success) {
        throw result.error
      }
      return result.data
